...
namespace StringJudge
{
    public partial class Main : Form
    {
        ...
        public Main()
        {
            ...
            #region 布局
            TlpLayout.RowCount = StringJudge.Layout.row;
            TlpLayout.ColumnCount = StringJudge.Layout.col;

            for (int i = 0; i < StringJudge.Layout.row; i++)
            {
                TlpLayout.RowStyles.Add(new System.Windows.Forms.RowStyle(System.Windows.Forms.SizeType.Percent, 100F));
            }
            for (int i = 0; i < StringJudge.Layout.col; i++)
            {
        ...
        private void TxtSN_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Return)
            {
                ...
                {
                    MessageBox.Show("马达号不能少于24位，请重新输入");
                    TxtSN.SelectAll();
                    return;                    
                }
        ...
        void Action(string SN)
        {
            ...
            if (sequence == StringJudge.Layout.sum) { BtnClear_Click(sender, new EventArgs()); }
            ...
            try
            {
                TlpLayout.Controls.Add(StringJudge.Layout.paint(AllInfo.SNlist[AllInfo.SNlist.Count - 1]),
        ...
        private void BtnTest_Click(object sender, EventArgs e)
        {
            ...
            if (isOne)
            {
                for (int i = 0; i < StringJudge.Layout.sum; i++)
                {

...
namespace NTRSjudge
{
    ...
    class Sum:Document
    {
        ...
        public static bool ChangeFileName()
        {
            string srcRelativePath= Document.pathList[2] + "total_yield.txt";
            string desRelativePath= Document.pathList[2] + "total_yield"+DateTime.Now.AddDays(-1).ToString("yyyyMMdd") +".txt";
            try
            {
            ...
            {
                MessageBox.Show(ex.Message,"文件操作",MessageBoxButtons.OK,MessageBoxIcon.Error);
                return false;
            }
        }
    }
